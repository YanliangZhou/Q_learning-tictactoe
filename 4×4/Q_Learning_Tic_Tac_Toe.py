...
class Game:
    def __init__(self, master, player1, player2, Q_learn=None, Q={}, alpha=0.3, gamma=0.9):
        ...
        for i in range(4):
            for j in range(4):
                self.buttons[i][j] = tk.Button(frame, height=4, width=10, text=self.empty_text, command=lambda i=i, j=j: self.callback(self.buttons[i][j]))
    ...
    def declare_outcome(self):
        if self.board.winner() is None:
            ...
            print(("{mark} won!".format(mark=self.current_player.mark)))
